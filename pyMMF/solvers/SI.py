...
def calc_mode(modes, idx, degenerate_mode, R, a, TH, Rlessa, Rgreatera):
    m = modes.m[idx]
    l = modes.l[idx]
    u = modes.u[idx]
    w = modes.w[idx]
    ...
    Et = phase_mult * (
        jv(m, u / a * R) / jv(m, u) * Rlessa + kn(m, w / a * R) / kn(m, w) * Rgreatera
    )
...
def associateLPModeProfiles(modes, indexProfile, degenerate_mode="sin", n_jobs=-2):
    """
    ...
    modes.profiles = Parallel(n_jobs=n_jobs)(
        delayed(calc_mode)(modes, idx, degenerate_mode, R, a, TH, Rlessa, Rgreatera)
